...
Dedicated full-edition tasks are producing Punjabi (Pakistan, Shahmukhi), Bengali (India), Marathi, Telugu, Tamil (India), Javanese, Gujarati and Pashto (Pakistan) editions against the same verified 722-file English source. Five edition repositories now have publicly byte-verified chapter releases:
...
| Marathi | [repository](https://github.com/KokunoYumeto/OpenLogic-mr-Deva-IN) · [release](https://github.com/KokunoYumeto/OpenLogic-mr-Deva-IN/releases/tag/sets-v0.1) | Complete Sets chapter, **7/722** source units. |
...
The [current cumulative readback](evidence/CUMULATIVE_PUBLIC_READBACK_20260904.json) verifies twelve GitHub assets for Gujarati, Bengali, Pashto and Tamil. [Pashto Zenodo readback](evidence/PASHTO_CUMULATIVE_ZENODO_READBACK_20260904.json) verifies six files, including the three preserved predecessor files. Marathi's initial release remains covered by the [earlier receipt](evidence/NEW_CHAPTER_PUBLIC_READBACK_20260904.json). These are **partial editions**, not completed books. Byte verification is not independent linguistic certification. Published source/reader scope follows the tagged edition records; later local drafts are not counted. Bengali's offline HTML has structural checks but browser visual inspection remains explicitly pending.

The [ownership register](catalogue/ownership.json) retains all eight full-edition assignments. Punjabi, Telugu and Javanese have source-aligned drafts but no release verified in this catalogue checkpoint. Bangladesh Bengali and Vietnamese remain additional, unassigned candidates. The [workflow evidence check](evidence/CANON_WORKFLOW_CHECK_20260904.json) records independent canon-file/reference checks and the recovered-canon handoff repair, with its limits stated explicitly.
